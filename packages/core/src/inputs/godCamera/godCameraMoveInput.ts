...
import type { GodCameraMoveInputKeyBindings } from "./godCameraMoveInputKeyBindings";
import type { GodCameraMoveInputParameters } from "./godCameraMoveInputParameters";
...
export class GodCameraMoveInput implements ICameraInput<GodCamera> {
  ...
  public attachControl(noPreventDefault?: boolean): void {
    ...
    for (const keyboard of this._deviceSourceManager.getDeviceSources(DeviceType.Keyboard)) {
    ...
    this._onDeviceDisconnectedObserver = this._deviceSourceManager.onDeviceDisconnectedObservable.add((device) => {
      const observer = this._onInputChangedObserverMap.get(device.deviceSlot);
      if (observer) {
        // biome-ignore lint/suspicious/noExplicitAny: observer is T extends KeyboardEvent only
        device.onInputChangedObservable.remove(observer as unknown as any);
